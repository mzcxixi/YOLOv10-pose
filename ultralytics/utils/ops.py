...
def v10postprocess(preds,key_point,max_det, nc=80):
    ...
    key_point = torch.gather(key_point, dim=1, index=index.repeat(1, 1, key_point.shape[-1]))
    ...
    key_point = key_point.gather(dim=1, index=index.unsqueeze(-1).repeat(1, 1, key_point.shape[-1]))
    return boxes, scores, labels,key_point